...
class Bacteria(Agent):
    ...
    def step(self):
        ...
        x_new = self.pos[0] + self.velocity * run_status * np.cos(np.deg2rad(self.ang)) * self.dt + np.sqrt(
            2 * D_rot * self.dt) * self.W_x
        y_new = self.pos[1] + self.velocity * run_status * np.sin(np.deg2rad(self.ang)) * self.dt + np.sqrt(
            2 * D_rot * self.dt) * self.W_y
